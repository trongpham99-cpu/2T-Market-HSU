...
exports.adminPostProduct = async (req, res) => {
  const { productName, productPrice, description, productAddress, loai_sp, status = "1", nguoi_dang_sp="admin",imagePath } = req.body;
...
//SAN PHAM ADMIN Đang Bán
exports.getSpAdmin = async (req,res)=>{
  res.send(await Profile
    .find({nguoi_dang_sp:'admin',status:'1'})
    .sort({
        ngay_dang: -1,
    })
    .limit(1000));
}
//SAN PHAM ADMIN Đã Bán 
exports.getAdminDaBan = async (req,res)=>{
  res.send(await Profile
    .find({nguoi_dang_sp:'admin',status:'2'})
    .sort({
        ngay_dang: -1,
    })
    .limit(1000));
...
exports.postProfile = async (req, res) => {
  ...
  const imagePath = 'http://192.168.1.4:8080/images/' + req.file.filename; // Note: set path dynamically
